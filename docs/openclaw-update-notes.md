...
- `host-control` plugin compatibility
- bridge reachability and auth assumptions
- local media handling for `host_control_*` flows
...
- managed OpenClaw plugin:
  - [host-control-openclaw-plugin](../host-control-openclaw-plugin)
- host enforcement layer:
  - [openclaw-host-bridge](../openclaw-host-bridge)

Canonical source-of-truth split:

- `host-control-openclaw-plugin/` is maintained in this repository
...
3. verify the managed `host-control` plugin still loads
...
- keep more host behavior in the `host-control` bridge/plugin layers
