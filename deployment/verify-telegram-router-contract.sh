...
for path in "${required_paths[@]}"; do
  if [[ ! -f "$path" ]]; then
    echo "Missing required router file: $path" >&2
    exit 1
  fi
done
...
if ! cmp -s "$DEPLOY_ROUTER" "$CANON_ROUTER"; then
  echo "Deployment Telegram router differs from canonical standalone repo." >&2
  echo "Run ./deployment/sync-telegram-build-copy.sh before release." >&2
  exit 1
fi
